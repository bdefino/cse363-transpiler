...
int
idgraph_fini(struct idgraph *graph);

/* compute a hash of the raw instruction */
static int
idgraph__ihash(size_t *dest, struct instruction *instruction);

int
idgraph_init(struct idgraph *dest, const size_t count,
	const struct instruction **instructions);

/* canonicalize the topological order of the instructions (into an array) */
int
idgraph_topology_canonicalize(const struct idgraph *graph,
...
*/
int
idgraph_topology_mutate(const struct idgraph *graph, struct instruction **dest,
	const size_t destlen, int (*selector)(const struct idgraph *,
		struct instruction *, const size_t,
		const struct instruction **));

/* randomize the topological order of the instructions (into an array) */
int
idgraph_topology_randomize(const struct idgraph *graph,
	struct instruction **dest, const size_t destlen);

/* selector for a random topological order */
static int
idgraph_topology_randomize__selector(const struct idgraph *graph,
